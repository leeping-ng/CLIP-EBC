...
def standardize_dataset_name(dataset: str) -> str:
    assert dataset.lower(
    ) in available_datasets, f"Dataset {dataset} is not available."
...
class Crowd(Dataset):
    ...
    def __make_dataset__(self) -> None:
        image_npys = glob(os.path.join(
            self.root, self.split, "images", "*.npy"))
        if len(image_npys) > 0:
            self.image_type = "npy"
            image_names = image_npys
        else:
            self.image_type = "jpg"
            image_names = glob(os.path.join(
                self.root, self.split, "images", "*.jpg"))

        label_names = glob(os.path.join(
            self.root, self.split, "labels", "*.npy"))
        image_names = [os.path.basename(image_name)
                       for image_name in image_names]
        label_names = [os.path.basename(label_name)
        ...
        if self.dataset == "sha":
            if self.split == "train":
                assert len(self.image_names) == len(
                    self.label_names) == 300, f"ShanghaiTech_A train split should have 300 images, but found {len(self.image_names)}."
            else:
                assert len(self.image_names) == len(
                    self.label_names) == 182, f"ShanghaiTech_A val split should have 182 images, but found {len(self.image_names)}."
        elif self.dataset == "shb":
            if self.split == "train":
                assert len(self.image_names) == len(
                    self.label_names) == 400, f"ShanghaiTech_B train split should have 400 images, but found {len(self.image_names)}."
            else:
                assert len(self.image_names) == len(
                    self.label_names) == 316, f"ShanghaiTech_B val split should have 316 images, but found {len(self.image_names)}."
        elif self.dataset == "nwpu":
            if self.split == "train":
                assert len(self.image_names) == len(
                    self.label_names) == 3109, f"NWPU train split should have 3109 images, but found {len(self.image_names)}."
            else:
                assert len(self.image_names) == len(
                    self.label_names) == 500, f"NWPU val split should have 500 images, but found {len(self.image_names)}."
        elif self.dataset == "qnrf":
            if self.split == "train":
                assert len(self.image_names) == len(
                    self.label_names) == 1201, f"UCF_QNRF train split should have 1201 images, but found {len(self.image_names)}."
            else:
                assert len(self.image_names) == len(
                    self.label_names) == 334, f"UCF_QNRF val split should have 334 images, but found {len(self.image_names)}."
        else:  # self.dataset == "jhu"
            if self.split == "train":
                assert len(self.image_names) == len(
                    self.label_names) == 2772, f"JHU train split should have 2772 images, but found {len(self.image_names)}."
            else:
                assert len(self.image_names) == len(
                    self.label_names) == 1600, f"JHU val split should have 1600 images, but found {len(self.image_names)}."
    ...
    def __getitem__(self, idx: int) -> Union[Tuple[Tensor, Tensor, Tensor], Tuple[Tensor, Tensor, Tensor, str]]:
        ...
        if self.transforms is not None:
            images_labels = [self.transforms(
                image.clone(), label.clone()) for _ in range(self.num_crops)]
        ...
        if idx in self.indices:
            density_maps = torch.stack([generate_density_map(
                label, image.shape[-2], image.shape[-1], sigma=self.sigma) for image, label in zip(images, labels)], 0)
        else:
            labels = None
            density_maps = None
...
class NWPUTest(Dataset):
    def __init__(
        ...
        if len(image_npys) > 0:
            ...
            self.image_type = "jpg"
            image_names = glob(os.path.join(
                self.root, "test", "images", "*.jpg"))

        image_names = [os.path.basename(image_name)
                       for image_name in image_names]
        assert len(
            image_names) == 1500, f"NWPU test split should have 1500 images, but found {len(image_names)}."
        image_names.sort(key=get_id)
        self.image_names = tuple(image_names)
    ...
    def __getitem__(self, idx: int) -> Union[Tensor, Tuple[Tensor, str]]:
        ...
        image, _ = self.transforms(
            image, label) if self.transforms is not None else (image, label)
